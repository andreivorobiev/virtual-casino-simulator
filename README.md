...
Packaged application release: `0.9.5.42`
...
This repository has advanced beyond the original v9.1.1 bootstrap snapshot. Packaged release v0.9.5.42 carries the accepted issue #333 owner-only enrollment-policy transaction. Only the current active platform owner can strictly read, preview, or apply policy; preview returns an audit-bound deterministic revision, and apply compares that exact revision inside the provider transaction before proposal, operational logging, policy mutation, or audit append. A successful apply atomically commits the exact policy plus bounded hash-linked opaque-actor change audit and returns the exact prior policy, consumed revision, and new revision for explicit application rollback. The strict provider-neutral security-document seam fails closed on malformed schema-owned bytes, duplicate JSON keys, access failures, audit mismatch, stale or ABA revision, and capacity failure without fallback, normalization, backup, read-side write, or mutation. Existing login, public signup and invitation enforcement, bounded operational decision logging, legacy non-mapping or unowned-schema environment fallback, frozen `/api/v1`, readiness policy, UI, OAuth/provider enablement, provider network, public exposure, credential, DNS, billing, and mail behavior remain unchanged. The release retains the MySQL schema-two/schema-three rollback bridge, with migration application held and production remaining at schema 2 so exact v0.9.5.41 remains eligible for application-only rollback. Schema-3 production migration, MySQL composite execution, receipt-table grant hardening, remaining enrollment readiness and UI, routes, games, Slots adoption, ledger behavior, provider scaling, and all-provider atomicity remain separately governed. Independently versioned modules continue to record compatible source changes. Current module revisions and requirement status are recorded in the canonical manifests and generated requirements document.
