...
This repository has advanced beyond the original v9.1.1 bootstrap snapshot. Packaged release v0.9.5.10 preserves committed ledger history during player creation, rejects destructive money-history replacement, bounds Blackjack and Baccarat table rules before persistence, and applies the guest-wallet freeze plus amount validation to the frozen v1 credit route. It retains the packaged non-shell production monitor handoff, compatibility-owned predecessor selection, exact v0.9.5.9 application-only rollback target, TiltSeven canonical Casino origin, private-invite security, MySQL schema 2, provider-disablement, and public-exposure gates. Independently versioned modules continue to record compatible source changes. Current module revisions and requirement status are recorded in the canonical manifests and generated requirements document.
