...
Packaged application release: `0.9.5.13`
...
This repository has advanced beyond the original v9.1.1 bootstrap snapshot. Packaged release v0.9.5.13 makes settlement replay storage-atomic across additional games, restores reliable one-shot CSRF bootstrap recovery, funds competitive Bingo sessions, hardens practice-game recovery, and brings the remaining governed Python and Node regression suites into CI. It retains the packaged non-shell production monitor handoff, compatibility-owned predecessor selection, exact v0.9.5.12 application-only rollback target, TiltSeven canonical Casino origin, private-invite security, MySQL schema 2, provider-disablement, and public-exposure gates. Independently versioned modules continue to record compatible source changes. Current module revisions and requirement status are recorded in the canonical manifests and generated requirements document.
