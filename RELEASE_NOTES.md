...
## Repository status archive — 2026-07-31

Moved verbatim from the `README.md` section "Current repository status" on 2026-07-31 (packaged release v0.9.5.46). The README now fronts durable design decisions; per-release status prose belongs to this file's release sections.

This repository has advanced beyond the original v9.1.1 bootstrap snapshot. Packaged release v0.9.5.46 carries the accepted bounded shared-wallet celebration and lifecycle slice from sole content PR #552. The application keeps the latest server-owned balance authoritative while decorating ordinary and threshold gains with bounded normal-motion resources, suppressing celebration on initial load and reduced motion, and settling loss immediately. Route disposal, overlap, pagehide, BFCache restoration, and remount cannot leave stale timers, nodes, classes, or wallet writes. Hosted evidence covers English and Russian, four governed viewports, ordinary and threshold gains, and normal and reduced motion. Qualification also aligns the offline PWA harness with the reconstructed document's lifecycle and reserves Teen Patti's mobile fixed-control lane without changing product PWA behavior or game math. Original PR #521 is ancestry-reachable through shell `f1b1826d`; PR #552 is the sole content integration, issue #74 remains open at P1/stack-rank 020, and no second #521 content merge occurred. The release changes no game result, economy, ledger, API, provider, public policy, database, or migration behavior. It retains the MySQL schema-two/schema-three rollback bridge, with migration application held and production remaining at schema 2 so exact v0.9.5.45 remains eligible for application-only rollback; database rollback remains prohibited. Independently versioned modules continue to record compatible source changes. Current module revisions and requirement status are recorded in the canonical manifests and generated requirements document.
