...
from casino.errors import ForbiddenError, NotFoundError, UnauthorizedError, ValidationError

# Define a complete synthetic Google configuration with the exact public environment contract.
GOOGLE_ENV = {"CASINO_OAUTH_ENABLED_GOOGLE": "true", "CASINO_GOOGLE_CLIENT_ID": "synthetic-client", "CASINO_GOOGLE_CLIENT_SECRET": "synthetic-secret", "CASINO_OAUTH_PUBLIC_BASE_URL": "https://casino.example.test"}
# Define a complete independent synthetic Facebook configuration.
FACEBOOK_ENV = {"CASINO_OAUTH_ENABLED_FACEBOOK": "true", "CASINO_FACEBOOK_APP_ID": "synthetic-app", "CASINO_FACEBOOK_APP_SECRET": "synthetic-secret", "CASINO_OAUTH_PUBLIC_BASE_URL": "https://casino.example.test"}
...
class FakeAdapter:
    ...
    def authorization_url(self, *, redirect_uri, state, nonce, pkce_challenge):
        ...
        # Return a fixed HTTPS URL with no retained state or provider configuration.
...
class OAuthServiceTests(unittest.TestCase):
    ...
    def service(self, environment=None):
        ...
        return OAuthService(environ={} if environment is None else environment, storage=self.storage, adapter_factory=lambda provider, _client_id, _client_secret: FakeAdapter(provider))
    ...
    # Read the newest persisted flow's state without exposing it in assertions or logs.
    def newest_state(self):
        # Load the isolated flow document through the storage interface.
        document = self.storage.read_document(FLOW_DOCUMENT_KEY, lambda: {"flows": []})
        # Return only the most recently created flow's opaque state.
        return document["flows"][-1]["state"]
