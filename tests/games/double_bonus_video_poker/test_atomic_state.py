...
"""Cross-process evidence for provider-atomic Double Bonus state. (DBVP-003, TEST-219)"""
...
# Import the production engine and service fingerprint boundary.
...
class DoubleBonusAtomicStateTests(unittest.TestCase):
    # Build one wager-proven draw-phase round without touching a wallet.
    def _initial_state(self) -> dict:
        # Start from the exact game-owned default document.
        state = engine.default_state()
        # Bind the prepared deal to one normalized request fingerprint.
        deal_fingerprint = service.request_fingerprint({"stage": "deal", "bet": 2.0})
        # Create one deterministic reload-safe round plan.
        round_state = engine.create_round("atomic-player", 2, "atomic-deal-0001", round_id="dbvp-atomic-round", created_at="2026-08-16T00:00:00Z", request_fingerprint=deal_fingerprint, fixture={"hand": ["AS", "AH", "AD", "2C", "3D"], "draw_pile": ["AC", "KH", "4C", "5D", "6S"]})
        ...
        round_state["opening_ledger_id"] = "led-wager-atomic"
        ...
        state["action_receipts"]["atomic-deal-0001"] = {"stage": "deal", "round_id": "dbvp-atomic-round", "request_fingerprint": deal_fingerprint}
...
game_service = service.DoubleBonusVideoPokerService()
...
if not go_path.exists():
    raise RuntimeError('Double Bonus atomic race release timed out')
action_id = f'atomic-draw-{mode}-0001'
fingerprint = service.request_fingerprint({'stage': 'draw', 'round_id': 'dbvp-atomic-round', 'hold': []})
round_state = engine.round_by_id(state, 'dbvp-atomic-round')
engine.draw_round(round_state, action_id, (), completed_at=f'2026-08-16T00:0{mode}:00Z', request_fingerprint=fingerprint)
state['action_receipts'][action_id] = {'stage': 'draw', 'round_id': 'dbvp-atomic-round', 'request_fingerprint': fingerprint}
...
"""
            ...
            self.assertNotIn("_double_bonus_atomic_baseline", final)
