...
"""Session isolation and provider-atomic service tests for issues #138 and #793."""
...
from casino.games.pai_gow_poker import api, engine, service
...
class PaiGowPokerApiTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.service = PaiGowPokerService(repository=self.repository, ledger_gateway=self.ledger, get_player=lambda player_id: {"player_id": player_id, "balance": self.ledger.balances[player_id]}, clock=lambda: "2026-07-24T00:00:00Z", seed_factory=lambda action_id: f"api:{action_id}", fixture_factory=lambda action_id: self.fixtures.get(action_id))
    ...
    def test_deal_conflict_and_rejected_ante_cleanup(self):
        ...
        empty_service = PaiGowPokerService(repository=empty_repository, ledger_gateway=empty_ledger, get_player=lambda player_id: {"player_id": player_id, "balance": empty_ledger.balances[player_id]}, clock=lambda: "2026-07-24T00:00:00Z", fixture_factory=lambda action_id: self.fixtures["deal-win"])
    ...
    def test_lost_ante_response_recovers_once(self):
        ...
        self.ledger.lost_response_types.add("PAI_GOW_POKER_ANTE_DEBIT")
        # Surface the injected post-commit response loss.
        with self.assertRaisesRegex(RuntimeError, "lost PAI_GOW_POKER_ANTE_DEBIT response"):
            # Issue one stable opening action.
            self.start("deal-lost-ante", ante=5)
        ...
        replayed = self.start("deal-lost-ante", ante=5)
        # Require one event and stable decision-ready identity across recovery and replay.
        ante_events = [event for event in self.ledger.events if event["transaction_type"] == "PAI_GOW_POKER_ANTE_DEBIT"]
    ...
    def test_lost_settlement_response_recovers_once(self):
        ...
        self.ledger.lost_response_types.add("PAI_GOW_POKER_SETTLEMENT_CREDIT")
        # Surface the injected post-commit response loss.
        with self.assertRaisesRegex(RuntimeError, "lost PAI_GOW_POKER_SETTLEMENT_CREDIT response"):
            # Issue one stable winning set action.
            self.call(f"/api/v1/games/pai-gow-poker/rounds/{started['round']['round_id']}/decisions", {"action_id": "decision-lost-settlement", "set": "house_way"})
        # Recover settlement proof through the normal state endpoint.
        recovered = self.service.state("session-player")
        # Replay the same terminal decision without another credit.
        replayed = self.call(f"/api/v1/games/pai-gow-poker/rounds/{started['round']['round_id']}/decisions", {"action_id": "decision-lost-settlement", "set": "house_way"})
        # Require one returned-token movement and one authoritative terminal result.
        settlement_events = [event for event in self.ledger.events if event["transaction_type"] == "PAI_GOW_POKER_SETTLEMENT_CREDIT"]
    ...
    # Prove stale fresh processes preserve siblings and one provider-winning set.
    def test_fresh_process_set_race_preserves_provider_winner(self):
        ...
        with tempfile.TemporaryDirectory() as temporary, ProcessRacePool() as process_pool:
            ...
            round_id = "pgp_0123456789abcdef01234567"
            # Prepare one deterministic setting-ready round that returns no payout.
            active_round = engine.create_round("session-player", 5, "deal-process", round_id=round_id, created_at="2026-08-15T00:00:00Z", request_fingerprint=deal_fingerprint, fixture=self.fixtures["deal-loss"])
            # Bind immutable ante evidence so both workers race only the movement-free terminal set.
...
from casino.games.pai_gow_poker.service import PaiGowPokerService, StateRepository
...
class RendezvousRepository:
    def load(self, player_id):
        ...
        if not release.exists():
            raise RuntimeError('Pai Gow Poker race release timed out')
        return state
    def update(self, player_id, mutator):
...
class NoLedger:
    ...
    def apply_once(self, **_kwargs):
        raise RuntimeError('unexpected Pai Gow Poker movement')
game = PaiGowPokerService(repository=RendezvousRepository(), ledger_gateway=NoLedger(), get_player=lambda player_id: {'player_id': player_id, 'balance': 195.0}, clock=lambda: '2026-08-15T00:01:00Z')
try:
    result = game.decide('session-player', 'pgp_0123456789abcdef01234567', {'action_id': action_id, 'set': 'house_way'})
...
"""
            ...
            for index in range(2):
                ...
                process = process_pool.spawn([sys.executable, "-c", worker_source, str(ready_path), str(release_path), f"set-process-{index}"], cwd=repository_root, env=environment, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            ...
            # Release the first worker to publish the winning set.
            ...
            # Require the first set to commit.
            self.assertEqual((workers[0][0].returncode, winner_output.strip()), (0, "PASS:set-process-0"), winner_error)
            ...
            self.assertEqual((persisted["active_round"], terminal["phase"], terminal["decision_action_id"], persisted["atomic_markers"]), (None, "settled", "set-process-0", ["seed", "concurrent"]))
