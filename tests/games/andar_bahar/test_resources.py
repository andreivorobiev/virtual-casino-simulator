"""Locale-resource coverage tests for GitHub issue #140."""
...
# Verify the Andar Bahar locale domains remain complete and leakage-free.
class AndarBaharResourceTests(unittest.TestCase):
    ...
    def load(self, locale):
        ...
        return json.loads((self.root / "web" / "i18n" / locale / "games" / "andar_bahar.json").read_text(encoding="utf-8"))
    ...
    def test_russian_domain_has_no_obvious_english_leakage(self):
        ...
        prose_values = [value for key, value in russian.items() if not key.startswith("ranks.") and not key.startswith("suits.")]
