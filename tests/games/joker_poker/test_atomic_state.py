...
"""Cross-process evidence for provider-atomic Joker Poker state. (JP-006, TEST-222)"""
...
# Import the production request fingerprint used by terminal draws.
from casino.games.joker_poker.service import request_fingerprint


# Prove stale processes publish through the provider-owned latest document.
class JokerPokerAtomicStateTests(unittest.TestCase):
    ...
    def _initial_state(self) -> dict:
        ...
        active = engine.create_round("atomic-player", 1, deal_id, initial_hand=["AS", "KS", "QS", "JS", "JK"], draw_pool=["2C", "3C", "4C", "5C", "6C"], round_id=round_id, created_at="2026-08-16T00:00:00Z", request_fingerprint=deal_fingerprint)
...
from casino.games.joker_poker import engine, service
mode = sys.argv[1]
ready_path = Path(sys.argv[2])
go_path = Path(sys.argv[3])
game_service = service.JokerPokerService()
...
if not go_path.exists():
    raise RuntimeError('Joker Poker atomic race release timed out')
action_id = f'atomic-draw-{mode}-0001'
fingerprint = service.request_fingerprint({'stage': 'draw', 'round_id': 'jp_atomic_round', 'holds': []})
round_state = engine.round_by_id(state, 'jp_atomic_round')
engine.draw(round_state, action_id, completed_at=f'2026-08-16T00:0{mode}:00Z', request_fingerprint=fingerprint)
state['action_receipts'][action_id] = {'stage': 'draw', 'round_id': 'jp_atomic_round', 'request_fingerprint': fingerprint}
