...
"""Cross-process evidence for provider-atomic Let It Ride state. (LIR-006, TEST-225)"""
...
class LetItRideAtomicStateTests(unittest.TestCase):
    # Build one second-decision round shared by both stale workers.
    def _initial_state(self) -> dict:
        ...
        round_item = engine.start_round(state, "atomic-player", 1, "atomic-deal-0001", seed="atomic-lir", round_id="lir_atomic_round", shoe_id="lir_atomic_shoe", created_at="2026-08-16T00:00:00Z")
        # Retain the immutable opening request fingerprint.
        state["requests"]["atomic-deal-0001"] = {"command": "rounds", "round_id": "lir_atomic_round", "wager": 1.0}
        # Mark the prepared wager intent already committed.
        opening_intent = round_item["ledger_intents"][0]
        # Store one stable ledger marker so the proof concerns terminal state only.
        state["ledger_actions"][opening_intent["action_id"]] = {"ledger_id": "atomic-wager-ledger", "transaction_type": opening_intent["transaction_type"], "round_id": "lir_atomic_round", "amount": -3.0}
        # Advance the first decision without preparing a refund.
        engine.advance_decision(state, "lir_atomic_round", "first", "ride", "atomic-first-0001", completed_at="2026-08-16T00:01:00Z")
        # Retain the immutable first-decision request fingerprint.
        state["requests"]["atomic-first-0001"] = {"command": "decision", "round_id": "lir_atomic_round", "stage": "first", "decision": "ride"}
        # Return the complete second-decision production state.
        return state
...
controller = api.LetItRideController()
...
if not go_path.exists():
    raise RuntimeError('Let It Ride atomic race release timed out')
action_id = f'atomic-second-{decision}-0001'
engine.advance_decision(state, 'lir_atomic_round', 'second', decision, action_id, completed_at=f'2026-08-16T00:0{1 if decision == "ride" else 2}:00Z')
state['requests'][action_id] = {'command': 'decision', 'round_id': 'lir_atomic_round', 'stage': 'second', 'decision': decision}
try:
    controller._save('atomic-player', state)
except ConflictError:
    print(f'CONFLICT:{decision}')
else:
...
"""
            ...
            for decision in ("ride", "pull"):
                # Allocate one readiness marker for this child.
                ready_path = Path(temporary) / f"ready-{decision}"
                # Start the exact interpreter with bounded pipe capture.
                process = process_pool.spawn([sys.executable, "-c", worker_source, decision, str(ready_path), str(go_path)], cwd=repository_root, env=environment, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            ...
            # Resolve the one provider-winning retained round.
            round_item = final["rounds"]["lir_atomic_round"]
            # Require the decision terminal with no second actionable phase.
            self.assertEqual(("settled", 2), (round_item["phase"], len(round_item["decisions"])))
            # Preserve the unrelated sibling through the winning provider-current update.
            self.assertEqual(["sibling"], final["atomic_markers"])
            # Retain opening, first, and exactly one winning second-decision request.
            self.assertEqual(3, len(final["requests"]))
            # Retain only the already-committed opening ledger marker before later recovery.
            self.assertEqual(1, len(final["ledger_actions"]))
            # Reject the private optimistic baseline from persistent JSON bytes.
            self.assertNotIn("_let_it_ride_atomic_baseline", final)
