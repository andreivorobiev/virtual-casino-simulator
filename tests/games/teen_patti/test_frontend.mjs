// Statically verify the issue #1033 Teen Patti lifecycle slice and paired locale resources.
...
const source = await readFile(path.join(root, 'web', 'games', 'teen_patti.js'), 'utf8');
// Read the formatted route stylesheet as UTF-8 text.
const stylesheet = await readFile(path.join(root, 'web', 'games', 'teen_patti.css'), 'utf8');
// Parse the English game-owned dictionary.
const english = JSON.parse(await readFile(path.join(root, 'web', 'i18n', 'en-US', 'games', 'teen_patti.json'), 'utf8'));
// Retain and parse the Russian dictionary for encoding and parity evidence.
const russianSource = await readFile(path.join(root, 'web', 'i18n', 'ru-RU', 'games', 'teen_patti.json'), 'utf8');
...
for (const marker of ['createGameLifecycle', 'lifecycle.mount(node, render)', 'lifecycle.unmount()', 'lifecycle.root()', 'lifecycle.isBusy()', 'lifecycle.setBusy(true)', "lifecycle.nextRequestId('deal')", "lifecycle.nextRequestId('decision')", "requestPrefix: 'tp'", "href: '/games/teen_patti.css'"]) assert.ok(source.includes(marker), marker);
...
assert.match(source, /post\(`\$\{API_ROOT\}\/rounds\/\$\{encodeURIComponent\(round\.round_id\)\}\/decisions`,\s*withCurrentPlayer\(\{ action_id: pendingDecisionId, decision \}\)\)/);
...
// Preserve unresolved decision identity reuse for the exact round and decision.
assert.match(source, /if \(!pendingDecisionId \|\| pendingDecisionContext\?\.round_id !== round\.round_id[\s\S]*pendingDecisionContext\?\.decision !== decision\)[\s\S]*pendingDecisionId = lifecycle\.nextRequestId\('decision'\);[\s\S]*pendingDecisionContext = \{ round_id: round\.round_id, decision \};/);
...
// Preserve the three Bonus tiers, six ordered hand ranks, route selectors, and public controls.
assert.match(source, /const BONUS_ORDER = \['trail', 'pure_sequence', 'sequence'\];/);
assert.match(source, /const RANK_ORDER = \['trail', 'pure_sequence', 'sequence', 'color', 'pair', 'high_card'\];/);
for (const marker of ['data-testid="teen-patti"', 'data-testid="teen-patti-result"', 'data-ante', 'data-deal', 'data-repeat', 'data-fold', 'data-play']) assert.ok(source.includes(marker), marker);
// Require representative route, card, decision, wager, paytable, result, repeat, and responsive rules after extraction.
for (const selector of ['.teenp {', '.tp-stage {', '.tp-row h4 {', '.tp-cards {', '.tp-cards.win {', '.tp-actions {', '.tp-btn.play {', '.tp-btn.fold {', '.tp-btn.deal {', '.tp-field input {', '.tp-pays {', '.tp-rank {', '.tp-result {', '.tp-repeat {', '@media (max-width: 900px)', '@media (max-width: 640px)', '@media (prefers-reduced-motion: reduce)']) assert.ok(stylesheet.includes(selector), selector);
// Preserve the exact desktop rail, action touch target, narrow reference reservation, and feedback-control width.
assert.match(stylesheet, /\.teenp\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0, 1fr\) 300px;/);
assert.match(stylesheet, /\.tp-btn\s*\{[\s\S]*?min-height:\s*44px;/);
assert.match(stylesheet, /@media \(max-width:\s*640px\)[\s\S]*?\.tp-pays,[\s\S]*?\.tp-rank\s*\{[\s\S]*?width:\s*calc\(100% - 160px\);/);
assert.match(stylesheet, /body:has\(\.teenp\) \.report-problem-fab\s*\{[\s\S]*?width:\s*144px;/);
