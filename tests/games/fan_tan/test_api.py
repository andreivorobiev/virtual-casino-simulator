...
class FakeLedgerGateway:
    ...
    def apply_once(self, *, player_id, amount, transaction_type, round_id, action_key, request_fingerprint, details):
        ...
        event = {"ledger_id": f"ledger-{len(self.events) + 1}", "player_id": player_id, "amount": amount, "transaction_type": transaction_type, "game": "fan_tan", "round_id": round_id, "details": copy.deepcopy(details), "ts": "2026-07-14T00:00:00Z"}
...
"""
    ...
    def test_legacy_debit_proof_recovery_uses_event_time_and_pile_count(self):
        ...
        self.ledger.apply_once(player_id="player-a", amount=-1.0, transaction_type="FAN_TAN_WAGER_DEBIT", round_id=round_id, action_key=f"{round_id}:wager", request_fingerprint=fingerprint, details={"action_id": request["action_id"], "request_fingerprint": fingerprint, "wagers": wagers, "pile_count": 49})
