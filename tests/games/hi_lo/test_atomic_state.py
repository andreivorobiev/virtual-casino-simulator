...
"""Cross-process evidence for provider-atomic Hi-Lo state. (HILO-006, TEST-223)"""
...
# Import the production request fingerprint used by terminal guesses.
...
class HiLoAtomicStateTests(unittest.TestCase):
    ...
    def _initial_state(self) -> dict:
        ...
        # Bind one stable prepared-round identity.
        round_id = "hi_lo_atomic_round"
        # Bind one stable deal identity for replay history.
        deal_id = "atomic-deal-0001"
        # Bind the deal to its normalized wager.
        deal_fingerprint = request_fingerprint({"stage": "deal", "wager": 1.0})
        # Create one deterministic active decision with a private higher card.
        active = engine.create_round("atomic-player", 1, deal_id, current_card="5H", next_card="9S", round_id=round_id, created_at="2026-08-16T00:00:00Z", request_fingerprint=deal_fingerprint)
        # Mark the wager already committed so both stale workers may guess.
        ...
        # Retain the durable deal receipt used by both workers.
        state["action_receipts"][deal_id] = {"stage": "deal", "round_id": round_id, "request_fingerprint": deal_fingerprint}
...
from casino.games.hi_lo import engine, service
mode = sys.argv[1]
ready_path = Path(sys.argv[2])
go_path = Path(sys.argv[3])
game_service = service.HiLoService()
...
if not go_path.exists():
    raise RuntimeError('Hi-Lo atomic race release timed out')
guess = 'higher' if mode == '1' else 'lower'
action_id = f'atomic-guess-{mode}-0001'
fingerprint = service.request_fingerprint({'stage': 'guess', 'round_id': 'hi_lo_atomic_round', 'guess': guess})
round_state = engine.round_by_id(state, 'hi_lo_atomic_round')
engine.settle_round(round_state, guess, action_id, completed_at=f'2026-08-16T00:0{mode}:00Z', request_fingerprint=fingerprint)
state['action_receipts'][action_id] = {'stage': 'guess', 'round_id': 'hi_lo_atomic_round', 'request_fingerprint': fingerprint}
...
"""
            ...
            # Retain the deal and exactly one winning guess identity.
            self.assertEqual(2, len(final["action_receipts"]))
            # Reject the private optimistic baseline from persistent JSON bytes.
            self.assertNotIn("_hi_lo_atomic_baseline", final)
