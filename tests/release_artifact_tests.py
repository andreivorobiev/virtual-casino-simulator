...
class ReleaseArtifactTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Define one exact schema-four immutable-claims fixture migration.
        migration_four = json.dumps({"version": 4, "name": "game-action-claims", "description": "fixture", "statements": ["CREATE TABLE fixture_game_action_claims (game_id VARCHAR(191) NOT NULL, player_id VARCHAR(191) NOT NULL, action_key VARCHAR(191) NOT NULL, disposition VARCHAR(16) NOT NULL, PRIMARY KEY (game_id, player_id, action_key))"]}, indent=2) + "\n"
        # Define one exact schema-five first-class-session fixture migration.
        migration_five = json.dumps({"version": 5, "name": "first-class-sessions", "description": "fixture", "statements": ["CREATE TABLE fixture_sessions (session_id VARCHAR(64) NOT NULL, token_digest CHAR(64) NOT NULL, user_id VARCHAR(191) NOT NULL, PRIMARY KEY (session_id), UNIQUE KEY fixture_sessions_token (token_digest))"]}, indent=2) + "\n"
        # Build the checksum-pinned fixture catalog from exact UTF-8 bytes.
        migration_catalog = json.dumps({"schema": "casino-mysql-migration-catalog-v1", "minimum_runtime_version": 2, "expected_version": 5, "apply_policy": "held", "migrations": [{"version": 1, "name": "initial", "file": "0001_initial.json", "sha256": hashlib.sha256(migration_one.encode("utf-8")).hexdigest()}, {"version": 2, "name": "upgrade", "file": "0002_upgrade.json", "sha256": hashlib.sha256(migration_two.encode("utf-8")).hexdigest()}, {"version": 3, "name": "game-action-receipts", "file": "0003_game_action_receipts.json", "sha256": hashlib.sha256(migration_three.encode("utf-8")).hexdigest()}, {"version": 4, "name": "game-action-claims", "file": "0004_game_action_claims.json", "sha256": hashlib.sha256(migration_four.encode("utf-8")).hexdigest()}, {"version": 5, "name": "first-class-sessions", "file": "0005_first_class_sessions.json", "sha256": hashlib.sha256(migration_five.encode("utf-8")).hexdigest()}]}, indent=2) + "\n"
        ...
        self.files = {
            ...
            "migrations/mysql/0005_first_class_sessions.json": migration_five,
    ...
    def test_current_release_compatibility_binds_private_invite_predecessor(self):
        ...
        compatibility = json.loads((package_app.ROOT / "contracts" / "compatibility" / "app-0.9.5.86.json").read_text(encoding="utf-8"))
        # Require the canonical release and restricted-preview channel identities.
        self.assertEqual((compatibility["app_version"], compatibility["release_channel"]), ("0.9.5.86", "restricted-preview-private-invite"))
        # Require the exact prior packaged release and retained manifest filename.
        self.assertEqual(
            compatibility["predecessor"],
            {
                "app_version": "0.9.5.85",
                "compatibility_record": "contracts/compatibility/app-0.9.5.85.json",
                "required_artifact": "release-manifest.json",
                "source_commit_sha": "54aa25723d68ee35f3617ba6b9e625ee3e6dd435",
                "artifact_sha256": "ddf0750eb953288529e49dc74b9409dc55030c821fcbf8a4a060ada16d591478",
                "manifest_sha256": "d611e848d9f7a5c4957964218c82f05536f9d54fd57139c90aaf714317e41874",
            },
        )
        ...
        self.assertEqual(resolve_release_predecessor.predecessor_tag("0.9.5.86"), "v0.9.5.85")
    ...
    def test_clean_extracted_copy_smoke(self):
        ...
        # Require release provenance to bind the schema-two-to-five bridge window.
        self.assertEqual((manifest["mysql_schema"]["minimum_version"], manifest["mysql_schema"]["expected_version"]), (2, 5))
