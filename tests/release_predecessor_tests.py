...
class ReleasePredecessorTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.write_json(
            "contracts/compatibility/app-0.9.5.60.json",
            {
                "app_version": "0.9.5.60",
                ...
                "predecessor": {
                    ...
                    "compatibility_record": "contracts/compatibility/app-0.9.5.59.json",
    ...
    def test_resolves_declared_predecessor_tag(self):
        ...
        tag = resolve_release_predecessor.predecessor_tag("0.9.5.60", self.root)
        # Require the retained v0.9.5.59 tag rather than release-list ordering.
        self.assertEqual(tag, "v0.9.5.59")
    ...
    def test_rejects_candidate_rollback_policy_drift(self):
        ...
        path = self.root / "contracts" / "compatibility" / "app-0.9.5.60.json"
        ...
        for field, value in mutations:
            ...
            with self.subTest(field=field), self.assertRaises(ValueError):
                ...
                resolve_release_predecessor.predecessor_tag("0.9.5.60", self.root)
    ...
    def test_verifies_exact_downloaded_manifest(self):
        ...
        tag = resolve_release_predecessor.verify_manifest("0.9.5.60", manifest, self.root)
    ...
    def test_rejects_cross_tag_manifest(self):
        ...
        with self.assertRaises(ValueError):
            ...
            resolve_release_predecessor.verify_manifest("0.9.5.60", manifest, self.root)
    ...
    def test_rejects_schema_three_to_exact_schema_two_predecessor(self):
        ...
        candidate_path = self.root / "contracts" / "compatibility" / "app-0.9.5.60.json"
        ...
        manifest = self.write_json("previous-manifest.json", {"app_version": "0.9.5.59", "source": {"release_tag": "v0.9.5.59", "commit_sha": "9" * 40}, "artifact": {"name": "virtual_casino_simulator_package.zip", "sha256": "8" * 64}, "mysql_schema": {"minimum_version": 2, "expected_version": 2}})
    ...
    def test_future_schema_three_candidate_accepts_bridge_predecessor(self):
        ...
        self.write_json("contracts/compatibility/app-0.9.5.61.json", {"app_version": "0.9.5.61", "rollback": {"scope": "application-only", "database_rollback": "prohibited", "mysql_expected_schema_version": 3, "requires_retained_predecessor_manifest": True}, "predecessor": {"app_version": "0.9.5.60", "compatibility_record": "contracts/compatibility/app-0.9.5.60.json", "required_artifact": "release-manifest.json", "source_commit_sha": "9" * 40, "artifact_sha256": "8" * 64, "manifest_sha256": ""}})
        # Write the bridge predecessor compatibility identity.
        self.write_json("contracts/compatibility/app-0.9.5.60.json", {"app_version": "0.9.5.60"})
        # Write a predecessor manifest whose runtime accepts schema three.
        manifest = self.write_json("bridge-manifest.json", {"app_version": "0.9.5.60", "source": {"release_tag": "v0.9.5.60", "commit_sha": "9" * 40}, "artifact": {"name": "virtual_casino_simulator_package.zip", "sha256": "8" * 64}, "mysql_schema": {"minimum_version": 2, "expected_version": 3, "apply_policy": "held"}})
        # Bind the exact manifest digest into future compatibility.
        future_path = self.root / "contracts" / "compatibility" / "app-0.9.5.61.json"
        ...
        self.assertEqual(resolve_release_predecessor.verify_manifest("0.9.5.61", manifest, self.root), "v0.9.5.60")
