                    ...
                    def localized_evidence(prefix,states):
                        ...
                        for locale in ('en-US','ru-RU'):
                            ...
                            for viewport_id,width,height in required_viewports:
                                ...
                                page.set_viewport_size({'width':width,'height':height}); page.wait_for_timeout(100)
                                # Collect document-fit and fixed-feedback overlap geometry for an actionable hosted failure.
                                paytable_geometry=page.evaluate("() => { const button=document.querySelector('.report-problem-fab:not([hidden])'); const fixed=button?.getBoundingClientRect(); const overlaps=fixed?[...document.querySelectorAll('.db-paytable h3,.db-paytable .db-pays > div')].filter(node=>{const row=node.getBoundingClientRect(); return row.left<fixed.right&&row.right>fixed.left&&row.top<fixed.bottom&&row.bottom>fixed.top;}).length:0; return {documentFits:document.documentElement.scrollWidth<=window.innerWidth+1,feedbackOverlaps:overlaps}; }")
                                ...
                                assert paytable_geometry=={'documentFits':True,'feedbackOverlaps':0} and mounted_geometry=={'gameVisible':True,'paytableRows':11,'paytableVisible':True},{'paytableGeometry':paytable_geometry,'mountedGeometry':mounted_geometry,'locale':locale,'viewport':viewport_id}
