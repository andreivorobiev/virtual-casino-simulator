            ...
            try:
                ...
                def pwa_installable_shell():
                    ...
                    try:
                        ...
                        assert registration_identity=={'ready':True,'state':'activated','scriptUrl':f'{base}/sw.js?v=9.5.4'},{'registration':registration_identity,'workerDiagnostics':pwa_worker_diagnostics[-4:]}
                        ...
                        pwa_page.wait_for_function("() => Boolean(navigator.serviceWorker.controller) && window.CasinoPwa?.version==='9.5.4'",timeout=8000)
                        ...
                        assert worker_identity['pageVersion']=='9.5.4' and worker_identity['controller'] and worker_identity['scriptUrl'].endswith('/sw.js?v=9.5.4'),worker_identity
                    ...
                    def localized_evidence(prefix,states):
                        ...
                        for locale in ('en-US','ru-RU'):
                            ...
                            for viewport_id,width,height in required_viewports:
                                ...
                                page.set_viewport_size({'width':width,'height':height}); page.wait_for_timeout(100)
                                # Collect document-fit and fixed-feedback overlap geometry for every card control and payout row.
                                card_geometry=page.evaluate("() => { const button=document.querySelector('.report-problem-fab:not([hidden])'); const fixed=button?.getBoundingClientRect(); const overlaps=fixed?[...document.querySelectorAll('.db-card h3,.db-card label,.db-card input,.db-card button,.db-card .db-pays > div')].filter(node=>{const row=node.getBoundingClientRect(); return row.left<fixed.right&&row.right>fixed.left&&row.top<fixed.bottom&&row.bottom>fixed.top;}).length:0; return {documentFits:document.documentElement.scrollWidth<=window.innerWidth+1,feedbackOverlaps:overlaps}; }")
                                ...
                                assert card_geometry=={'documentFits':True,'feedbackOverlaps':0} and mounted_geometry=={'gameVisible':True,'paytableRows':11,'paytableVisible':True},{'cardGeometry':card_geometry,'mountedGeometry':mounted_geometry,'locale':locale,'viewport':viewport_id}
