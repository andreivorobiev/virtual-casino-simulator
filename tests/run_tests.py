                ...
                def admin_users_browser():
                    ...
                    for locale in ('en-US','ru-RU'):
                        ...
                        for viewport_id,viewport in users_viewports.items():
                            ...
                            page.set_viewport_size(viewport); page.wait_for_timeout(100)
                            # Measure containment and readable inline size independently from screenshot success.
                            handoff_geometry=handoff.evaluate("""element => { const rect=element.getBoundingClientRect(); return { documentContained: document.documentElement.scrollWidth <= window.innerWidth + 1, regionContained: rect.left >= -1 && rect.right <= window.innerWidth + 1, readable: rect.width >= Math.min(320, window.innerWidth - 32), controlContained: [...element.querySelectorAll('button')].every(control => { const box=control.getBoundingClientRect(); return box.left >= rect.left - 1 && box.right <= rect.right + 1 && box.width >= 44 && box.height >= 36; }) }; }""")
                            # Fail closed on document overflow, clipped content, narrow copy, or an unusable handoff control.
                            assert all(handoff_geometry.values()),handoff_geometry
