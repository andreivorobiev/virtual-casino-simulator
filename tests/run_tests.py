            ...
            try:
                ...
                def invitation_browser():
                    ...
                    for locale in ('en-US','ru-RU'):
                        ...
                        page.set_viewport_size(viewports['mobile']); page.evaluate("() => { document.body.style.zoom='200%'; document.body.style.width='50%'; }"); page.wait_for_timeout(100)
                        # Measure the page, panel, form, and every visible enrollment field at the exact zoom state.
                        public_zoom_geometry=page.evaluate("() => { const panel=document.querySelector('[data-testid=\"invitation-redemption\"]'); const form=document.querySelector('#invitation-form'); const panelBox=panel.getBoundingClientRect(); const formBox=form.getBoundingClientRect(); const controls=[...form.querySelectorAll('input, select, button')].map(control => ({ testId:control.getAttribute('data-testid'), clientWidth:control.clientWidth, scrollWidth:control.scrollWidth, box:control.getBoundingClientRect().toJSON() })); return { viewport:innerWidth, documentScrollWidth:document.documentElement.scrollWidth, panelClientWidth:panel.clientWidth, panelScrollWidth:panel.scrollWidth, panelBox:panelBox.toJSON(), formClientWidth:form.clientWidth, formScrollWidth:form.scrollWidth, formBox:formBox.toJSON(), controls }; }")
                        # Require page/panel/form containment plus every native field fully painted inside the form.
                        assert public_zoom_geometry['documentScrollWidth']<=public_zoom_geometry['viewport']+1 and public_zoom_geometry['panelScrollWidth']<=public_zoom_geometry['panelClientWidth']+1 and public_zoom_geometry['formScrollWidth']<=public_zoom_geometry['formClientWidth']+1 and public_zoom_geometry['panelBox']['left']>=-1 and public_zoom_geometry['panelBox']['right']<=public_zoom_geometry['viewport']+1 and all(control['scrollWidth']<=control['clientWidth']+1 and control['box']['left']>=public_zoom_geometry['formBox']['left']-1 and control['box']['right']<=public_zoom_geometry['formBox']['right']+1 for control in public_zoom_geometry['controls']), {'locale':locale,'viewport':'mobile','zoom':200,'geometry':public_zoom_geometry}
