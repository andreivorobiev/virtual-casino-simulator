...
class MultiprocessSafetyInventoryTests(unittest.TestCase):
    ...
    def test_all_registered_games_are_reachably_classified_and_blocked(self) -> None:
        ...
        # Resolve Let It Ride after every prepared, decision, marker, recovery, and rollback publication becomes atomic.
        let_it_ride = next(row for row in games if row["game_id"] == "let_it_ride")
        # Require authoritative reads for response, replay, and ledger recovery.
        self.assertGreater(let_it_ride["load_call_sites"], 0)
        # Require every reachable game-state publication to avoid stale whole-document saves.
        self.assertEqual(let_it_ride["save_call_sites"], 0)
        # Bind preparation, staged decisions, terminal state, recovery markers, and rollback to provider updates.
        self.assertGreater(let_it_ride["atomic_update_call_sites"], 0)
        # Name completed state serialization without claiming wallet-state atomicity.
        self.assertEqual(let_it_ride["state_model"], "provider_atomic_player_document")
        # Keep production second-worker activation blocked while money is a separate transaction.
        self.assertEqual(let_it_ride["multiworker_status"], "blocked")
        # Resolve Mississippi Stud after preparation, streets, recovery, settlement, and rollback become atomic.
        mississippi_stud = next(row for row in games if row["game_id"] == "mississippi_stud")
        # Require authoritative reads for response, replay, and ledger recovery.
        self.assertGreater(mississippi_stud["load_call_sites"], 0)
        # Require every reachable game-state publication to avoid stale whole-document saves.
        self.assertEqual(mississippi_stud["save_call_sites"], 0)
        # Bind deals, streets, markers, recovery, terminal archive, and rollback to provider updates.
        self.assertGreater(mississippi_stud["atomic_update_call_sites"], 0)
        # Name completed state serialization without claiming wallet-state atomicity.
        self.assertEqual(mississippi_stud["state_model"], "provider_atomic_player_document")
        # Keep production second-worker activation blocked while money is a separate transaction.
        self.assertEqual(mississippi_stud["multiworker_status"], "blocked")
        # Resolve Plinko after preparation, recovery markers, history, receipts, and rollback become atomic.
        plinko = next(row for row in games if row["game_id"] == "plinko")
        # Require authoritative reads for response, replay, and ledger recovery.
        self.assertGreater(plinko["load_call_sites"], 0)
        # Require every reachable game-state publication to avoid stale whole-document saves.
        self.assertEqual(plinko["save_call_sites"], 0)
        # Bind preparation, receipts, recovery markers, terminal history, and rollback to provider updates.
        self.assertGreater(plinko["atomic_update_call_sites"], 0)
        # Name completed state serialization without claiming state-and-money atomicity.
        self.assertEqual(plinko["state_model"], "provider_atomic_player_document")
        # Keep production second-worker activation blocked while money is a separate transaction.
        self.assertEqual(plinko["multiworker_status"], "blocked")
        # Resolve Red Dog after preparation, requests, recovery markers, decisions, and rollback become atomic.
        red_dog = next(row for row in games if row["game_id"] == "red_dog")
        # Require authoritative reads for response, replay, and ledger recovery.
        self.assertGreater(red_dog["load_call_sites"], 0)
        # Require every reachable game-state publication to avoid stale whole-document saves.
        self.assertEqual(red_dog["save_call_sites"], 0)
        # Bind opening and decision preparation, recovery markers, terminal state, and rollback to provider updates.
        self.assertGreater(red_dog["atomic_update_call_sites"], 0)
        # Name completed state serialization without claiming state-and-money atomicity.
        ...
        # Resolve Sic Bo after preparation, recovery markers, settlement, and cleanup become atomic.
        sic_bo = next(row for row in games if row["game_id"] == "sic_bo")
        # Require authoritative reads for response, replay, and ledger recovery.
        self.assertGreater(sic_bo["load_call_sites"], 0)
        # Require every reachable game-state publication to avoid stale whole-document saves.
        self.assertEqual(sic_bo["save_call_sites"], 0)
        # Bind private dice, ledger markers, terminal history, and cleanup to provider updates.
        self.assertGreater(sic_bo["atomic_update_call_sites"], 0)
        # Name completed state serialization without claiming state-and-money atomicity.
