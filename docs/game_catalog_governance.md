...
The v0.9.5.23 release line retains 46 catalog-discovered playable game descriptors and the accepted runtime-inert settings rule-schema gate while accelerating repository qualification only. The historical `GAME_CATALOG_TARGET = 20` value remains a release-readiness floor and reporting target, not a cap and not an instruction to add another catalog mechanism.
