...
The v0.9.5.17 release line retains 46 catalog-discovered playable game descriptors on the accepted catalog-reconciliation baseline. The historical `GAME_CATALOG_TARGET = 20` value remains a release-readiness floor and reporting target, not a cap and not an instruction to add another catalog mechanism.
