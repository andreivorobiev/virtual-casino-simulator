...
Every push to protected `main` runs `.github/workflows/deploy-production.yml`. A read-only preflight makes unchanged packaged versions successful no-ops; only an exact reviewed release-only wrapper reaches the conditional publisher. Root batches accepted main changes every three hours through the procedure in the linked runbook. Publication verifies the unique packaged identity, exact protected-main SHA, checksum-pinned predecessor and immutable hosted assets. The independent release-event lane is read-only verification, and the required publication aggregate fails when a prerequisite fails.
