...
Build a canonical tagged v0.9.5.38 candidate with the retained v0.9.5.37 rollback manifest selected by compatibility policy:

```powershell
python scripts/make_release.py --release-tag v0.9.5.38 --previous-manifest previous/release-manifest.json
```

For v0.9.5.38, `previous/release-manifest.json` must be the checksum-verified retained v0.9.5.37 release manifest declared by `contracts/compatibility/app-0.9.5.38.json`. `scripts/resolve_release_predecessor.py` derives that exact tag and verifies the downloaded manifest before packaging. The resulting v0.9.5.38 pointer authorizes application-artifact rollback only; it neither rolls back MySQL schema version 2 nor permits credential rotation, provider, DNS, billing, signup, OAuth, mail, edge, SSH-ingress, or public-exposure changes.
