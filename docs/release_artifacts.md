...
Build a canonical tagged v9.5.5 candidate with the retained immediate v9.5.4 rollback manifest:

```powershell
python scripts/make_release.py --release-tag v9.5.5 --previous-manifest previous/release-manifest.json
```

For v9.5.5, `previous/release-manifest.json` must be the checksum-verified retained v9.5.4 release manifest. The resulting pointer authorizes application-artifact rollback only; it neither rolls back MySQL schema version 2 nor permits deployment, edge activation, or public exposure. The protected v9.2.0 predecessor-recovery path remains a historical, one-time bootstrap control and is not used for ordinary v9.5.5 publication.
