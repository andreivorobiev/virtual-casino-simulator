...
The compatibility record, not GitHub release-list ordering, selects the retained predecessor. A published release with incorrect rollback provenance or an incomplete host-command inventory remains immutable and is superseded by a new patch identity; its assets and tag are never replaced in place. v0.9.5.32 retains the exact checksum-verified v0.9.5.31 release as its application-only predecessor while carrying exact visible-rank Hi-Lo pricing. v0.9.5.31 in turn retains v0.9.5.30 for independent Andar/Bahar side pricing.
...
Build a canonical tagged v0.9.5.32 candidate with the retained v0.9.5.31 rollback manifest selected by compatibility policy:

```powershell
python scripts/make_release.py --release-tag v0.9.5.32 --previous-manifest previous/release-manifest.json
```

For v0.9.5.32, `previous/release-manifest.json` must be the checksum-verified retained v0.9.5.31 release manifest declared by `contracts/compatibility/app-0.9.5.32.json`. `scripts/resolve_release_predecessor.py` derives that exact tag and verifies the downloaded manifest before packaging. The resulting v0.9.5.32 pointer authorizes application-artifact rollback only; it neither rolls back MySQL schema version 2 nor permits credential rotation, provider, DNS, billing, signup, OAuth, mail, edge, SSH-ingress, or public-exposure changes.
