...
Requirements `STORAGE-022` and `TEST-254` define the repository's PostgreSQL 16 schema boundary. The migration runner is intentionally limited to newly created, explicitly authorized, loopback-only disposable targets. It is not a production migration tool and this guide does not authorize an existing database, cloud target, service, listener, firewall, grant, release, or deployment change.

## Immutable schema catalog

`migrations/postgres/catalog.json` is the only executable PostgreSQL catalog. It declares `apply_policy=disposable-only`, minimum runtime version 5, expected version 5, and five contiguous checksum-bound JSON descriptors:
...
`apply` is available only after the complete disposable-target guard. It obtains a target-scoped PostgreSQL session advisory lock, initializes the two control tables only on an empty target, writes a committed `applying` marker before each descriptor, applies each descriptor in its own transaction, records its exact checksum, and returns the state to `clean`. A descriptor failure rolls back its DDL transaction and records the exact dirty version; subsequent execution fails closed and requires a separately reviewed forward-fix packet. Lock release is verified before success.
