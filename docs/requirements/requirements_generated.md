...
- **TEENP-001** (Teen Patti) - PASS: Teen Patti Practice is a stateful three-card poker played against the dealer. The player antes, sees three cards, then folds and forfeits the ante or makes a fixed Play wager equal to the ante. The dealer qualifies with a queen-high hand or better; if the dealer does not qualify the ante pays even money and the Play pushes, and if the dealer qualifies the best three-card hand wins both wagers even money with ties pushing. Teen Patti ranks a trail highest, then a pure sequence, a sequence, a colour, a pair, and a high card, so a sequence outranks a colour because runs are scarcer with three cards. A Bonus pays on the player's own trail, pure sequence, or sequence regardless of the dealer. The dealer's structural qualification keeps the game house-positive near a 3.4 percent edge on the ante under optimal play. Deal and decision are exactly-once and reload-safe on the shared ledger and player-scoped state store.
- **TEENP-002** (Teen Patti) - PASS: Teen Patti Practice inherits the exactly-once, reload-safe orchestration of the shared ledger: one caller-stable action id maps to one deal or decision, a retry replays the identical committed round and payout without moving the wallet again, and an action id reused with different wager content fails closed. The dealer's three cards stay private until the showdown, and a fold reveals only the player's own hand. The additive v1 contract exposes only the game-owned state, rounds, and decisions routes and never alters the frozen shared API. The published rules report the dealer qualification, the Bonus paytable, and the strongest-first hand ranking.
- **TEST-115** (Tests) - PASS: Listener-free evidence verifies Teen Patti: the three-card ranking places a trail highest and a sequence above a colour and recognizes the ace-two-three run, a played trail against a qualifying dealer wins the ante, play, and trail Bonus, a non-qualifying dealer pays the ante and pushes the play, the Bonus pays a sequence even in a losing showdown, a fold forfeits only the ante without revealing the dealer, a replayed decision is idempotent, a conflicting action-id reuse fails closed, a reload recovers the committed deal without a duplicate debit, invalid antes and decisions are rejected, and a large seeded sample confirms the near-optimal strategy leaves the game house-positive.
