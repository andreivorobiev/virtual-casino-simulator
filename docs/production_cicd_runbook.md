...
Packaged release numbers use the four-part scheme documented in [the release versioning policy](release_versioning.md). The current line is `0.9.5.82`; `0.9.6.0` remains reserved for a separately accepted product wave.
...
v0.9.5.82 is the first ordinary application wave delivered after #732's host pull poller reached terminal-green production operation on v0.9.5.81. Protected-main Actions publish the exact immutable assets and make no host connection; the enabled poller discovers the stable release, verifies checksums, provenance, inventory, clean schema 2, health, and readiness, then switches atomically with exact v0.9.5.81 retained for application-only rollback. Database rollback, migration application, grant mutation, provider activation, and public-policy changes remain prohibited.
