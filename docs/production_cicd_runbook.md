...
3. A changed version must be the next compatible patch and the exact result of one merged, main-targeting `Release-only: yes` PR on its canonical release branch. Both commit-to-pull observations must contain exactly that one canonical association. At least one current-head `APPROVED` review must come from a human distinct from the PR author and repository owner who still has provider-observed `write` or `admin`; a latest current-head `CHANGES_REQUESTED` blocks, while dismissed or otherwise nonqualifying approvals do not satisfy the gate. The semantic wrapper allowlist rejects unrelated source, API, provider, migration, test behavior, descriptor topology, or policy changes.
4. The conditional publisher binds exact-head operational receipts for the fixed `Senior B` and `Worker10` roles to the pre-merge metadata digest and wrapper tree. Those same-owner-login receipts must be unedited and strictly predate the merge, but are supplemental operational evidence only: they are neither the provider-distinct GitHub review approval nor release authorization. The publisher then rechecks the complete bounded admission snapshot and exact current main inside the shared publication lock. Creation requires both an authoritative Release API 404 and an absent peeled tag. A complete exact-head release can be verified and reused; tag conflicts, drafts, partial uploads, authentication/rate-limit/network ambiguity, or malformed responses stop publication.
...
6. It builds a new exact-head candidate through the existing complete release driver and publishes its three immutable assets once, or downloads the already complete exact-head release without uploading replacements. Immediately before mutation and again after publication, it re-observes the exact admission fingerprint; replacement evidence or moved main fails closed.
7. A direct post-writer verifier in the same non-cancellable queue downloads and verifies all current and predecessor assets, checksums, manifests, package provenance, and rollback evidence, then repeats the stable hosted observation. These repeated observations prove equality during the bounded workflow interval; they do not claim that the provider makes release metadata immutable. The workflow owns publication and never opens an inbound production connection.
...
12. After the current trusted verifier authenticates the files-only archive, it rejects links and special nodes in the extracted tree and normalizes only its real directories to `0755`. This is required because the poller service's `UMask=0027` otherwise makes root-owned `unzip` directories `0750`, which the unprivileged `casino` service cannot traverse; regular-file bytes and modes remain unchanged.
13. It stages the archive under `/opt/casino/releases/<commit-sha>`, writes a candidate `release.env`, captures the current selector and fragment, then atomically repoints `/opt/casino/current`.
14. It restarts Casino, reloads nginx, rechecks exact schema `2`, runs the authenticated edge observation, and requires `/healthz` to be live plus `/readyz` to report the exact candidate application version and build SHA.
15. If any post-switch check fails, it restores the prior application symlink and `release.env`, restarts the service, re-observes the prior release, and writes a durable alarm. Database rollback is prohibited.
16. After success, it atomically refreshes `/usr/local/libexec/casino-release-poller` from the verified active release and clears the alarm.
...
The legacy-named `Publish exact-main release` job is the post-merge workflow aggregate, not a pull-request merge requirement, and it is absent from branch protection. It reports success for exactly two tuples: successful intent plus `noop` with writer and verifier skipped, or successful intent plus `publish` with writer and verifier successful. A failed, cancelled, missing or inconsistent prerequisite cannot report green. The `codex_review_placeholder` context is retained only as status-name compatibility and echo evidence; it is never a human review, an approval, or release authorization.

The `release: published` trigger in `release.yml` is supplemental read-only verification for an externally initiated published-release event. Repository `GITHUB_TOKEN` publication cannot trigger that workflow, so it contributes nothing to the direct aggregate and is never primary publication proof. It uses the `immutable-release` environment only for its external-event admission and never rebuilds, uploads, replaces, deletes, or publishes assets.

Every provider list used for admission is deliberately required to fit on the requested first page (`<100` records, with exact inventories where specified). Hitting the cap is an availability HOLD rather than a claim of pagination completeness.

## Current provider prerequisites and HOLD (#1095)

Repository correction does not itself change provider settings. Before the first release-wrapper merge, an owner must separately accept and apply all of these settings as one coordinated provider change:

- onboard at least one non-owner human collaborator who can retain `write` or `admin` permission and review the exact wrapper head;
- atomically add `browser_tests` and `Build unpublished candidate` to branch protection, require one approving review, dismiss stale approvals, require approval of the last push, enforce the rule for administrators, and keep bypasses empty;
- add an active no-bypass tag ruleset covering `refs/tags/v0.9.5.*` with update and deletion prohibited while allowing the governed creation path; and
- admit `v0.9.5.*` to the `immutable-release` environment used only by the supplemental external-event verifier. The direct verifier intentionally has no environment because its caller ref is protected `main`.

Until the non-owner reviewer and all provider settings are independently observed and accepted, the production cadence remains **HOLD**. This correction authorizes no provider-setting mutation, release, deployment, production connection, database change, API change, or contract broadening.
...
The sole runtime-metadata exception is the required `docs/releases/whats_new.json` catalog: every entry must be strictly inactive, and wrapper authority is limited to byte-exact synchronization of each prior packaged-version field to the next packaged version. Catalog keys, copy, shape, formatting, adjacent What's New documentation and near-miss paths remain immutable; this exception does not activate or complete issue #165.
...
5. Install `casino-release-poller.service.template` and `casino-release-poller.timer.template` as `/etc/systemd/system/casino-release-poller.service` and `.timer`; replace the edge-monitor service with its same-release template. Verify both service units contain `Environment=PYTHONDONTWRITEBYTECODE=1`. Keep `/etc/casino/release-poller.env` absent for anonymous public access or create it root-owned mode `0640` with only the optional values above.
6. Run `systemd-analyze verify` on all three service/timer files, then `systemctl daemon-reload`. Do not enable the timer yet.
7. While the new immutable release is newer than the installed application, execute `sudo /usr/local/libexec/casino-release-poller rollback-drill` exactly once. The direct command reads only the allowlisted authorization and public-origin assignments from `/etc/casino/edge-monitor.env` when systemd has not supplied them; it never shell-sources or prints that file. The drill must activate and health-check the candidate, restore the exact predecessor, recheck schema `2` and authenticated health, leave the predecessor live, log `decision=rollback_drill` without an alarm, and leave no `__pycache__` directory or `.pyc` file under the candidate release root.
...
v0.9.5.84 packages accepted protected-main changes since v0.9.5.83, including shared frontend lifecycle adoption, browser/process reliability, version-governance ratchets, and first-class session-storage capacity. The schema-five migration descriptor is packaged but held; production and application-only rollback remain exact schema 2, and no database migration, grant mutation, provider activation, or database rollback is authorized. Its compatibility record retains exact terminal-green v0.9.5.83 as the application-only predecessor.

v0.9.5.83 supersedes v0.9.5.82 for production delivery. Its poller rejects links and special nodes in the authenticated extracted tree and sets only real directories to mode `0755` before candidate verification or selector mutation, preserving regular-file bytes and modes. The replacement retains exact v0.9.5.81 as its application-only predecessor, exact schema 2, and unchanged persistence; database rollback, migration application, and grant mutation remain prohibited.

v0.9.5.82 is the first ordinary application wave published after #732's host pull poller reached terminal-green production operation on v0.9.5.81. Its first two host polls verified exact public assets and schema 2, then failed closed when the root-owned poller extracted candidate directories as `0750` under `UMask=0027`; `casino.service` could not traverse its working directory, and the poller restored exact v0.9.5.81 both times without persistence or database mutation. Issue #987 owns the directory-only normalization repair and replacement release. Database rollback, migration application, grant mutation, provider activation, and public-policy changes remain prohibited.

v0.9.5.81 supersedes v0.9.5.80 for production completion after v0.9.5.80 activated and passed rollback, persistence, schema, service, readiness, public, and security gates but its first fresh edge-monitor invocation exposed a lag-check cleanup trap that survived beyond its function-local path. The replacement poller isolates each lag check in a subshell and owns its temporary-directory cleanup with a scoped EXIT trap. The v0.9.5.81 installation retained exact schema 2 and persistence, proved authenticated readiness at the release SHA, cleared the lag alarm, left both timers active, and removed only audited task-owned residue. Its compatibility record retains exact immutable v0.9.5.80 as the application-only predecessor; database rollback, migration application, and grant mutation remain prohibited.

v0.9.5.80 supersedes v0.9.5.79 for production delivery after the first v0.9.5.79 pull failed closed before activation because direct rollback-drill Python probes created bytecode cache files inside the otherwise byte-identical immutable release root. The replacement poller and both systemd units disable Python bytecode writes before every probe, and disposable evidence proves a rollback drill followed by an ordinary same-release poll leaves the verified release root exact while an unrelated extra file still fails closed. v0.9.5.79 is never retried; the v0.9.5.80 activation retained exact schema 2 and persistence before the separately repaired lag-monitor cleanup failure. Its compatibility record retains exact immutable v0.9.5.79 as the application-only predecessor; database rollback, migration application, and grant mutation remain prohibited.
