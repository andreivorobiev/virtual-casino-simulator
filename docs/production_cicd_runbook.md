...
Packaged release numbers use the four-part scheme documented in [the release versioning policy](release_versioning.md). The current line is `0.9.5.81`; `0.9.6.0` remains reserved for a separately accepted product wave.
...
v0.9.5.81 supersedes v0.9.5.80 for production completion after v0.9.5.80 activated and passed rollback, persistence, schema, service, readiness, public, and security gates but its first fresh edge-monitor invocation exposed a lag-check cleanup trap that survived beyond its function-local path. The replacement poller isolates each lag check in a subshell and owns its temporary-directory cleanup with a scoped EXIT trap. The v0.9.5.81 installation must retain exact schema 2 and persistence, prove authenticated readiness at the release SHA, clear the lag alarm, leave both timers active, and remove only the audited task-owned residue. Its compatibility record retains exact immutable v0.9.5.80 as the application-only predecessor; database rollback, migration application, and grant mutation remain prohibited.
