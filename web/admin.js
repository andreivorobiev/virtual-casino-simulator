...
async function requirements() {
  ...
  view.innerHTML = html`<section class="admin-card"><h3>${safe(t('nav.requirements', {}, 'admin'))}</h3>${table(['ID', 'Module', 'Description', 'Status', 'Tests'], (data.requirements || []).map(req => html`<tr><td>${safe(req.id)}</td><td>${safe(req.module)}</td><td>${safe(req.description)}</td><td>${safe(req.status)}</td><td>${safe([...(req.api_tests || []), ...(req.browser_tests || [])].join(', '))}</td></tr>`))}</section>`;
}
