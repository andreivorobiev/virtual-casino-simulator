...
function idleStage() {
  ...
  return `<p class="tp-result" data-testid="teen-patti-result">${safe(text('result.idle'))}</p>`;
}
...
function decisionStage(round) {
  ...
  return `${cards}<p class="tp-result">${safe(text('result.decide'))}</p><div class="tp-actions"><button class="tp-btn fold" data-fold="1" type="button" ${busy ? 'disabled' : ''}>${safe(text('action.fold'))}</button><button class="tp-btn play" data-play="1" type="button" ${busy ? 'disabled' : ''}>${safe(text('action.play'))}</button></div>`;
}
...
function settledStage(round) {
  ...
  return `${player}${dealer}<p class="tp-result" data-testid="teen-patti-result">${line}</p><div class="tp-actions"><button class="tp-btn deal" data-deal="1" type="button" ${busy ? 'disabled' : ''}>${safe(text('action.deal_again'))}</button></div>`;
}
