...
function render() { if (!root) return; root.innerHTML = `${repeatSafeStyleHtml()}${validationStyleHtml()}<div class="game-layout three-col stable-game slots-premium" data-testid="slots-premium">${controlRailHtml()}${stageHtml()}${drawerHtml()}</div>`; bindControls(); mountPaylineGeometry(); }
