...
export const SlotsGame = { id: 'slots', label: 'Slots', async mount(node) { root = node; localeUnsubscribe = onLocaleChange(() => render()); await load(); }, unmount() { if (autoBox?._stop) autoBox._stop(); if (localeUnsubscribe) localeUnsubscribe(); if (paylineResizeObserver) paylineResizeObserver.disconnect(); if (paylineFrame !== null) window.cancelAnimationFrame(paylineFrame); localeUnsubscribe = null; paylineResizeObserver = null; paylineFrame = null; root = null; } };
