...
// Define cellWin to highlight winning symbols without altering reel layout.
function cellWin(row, col) { return Boolean(firstLineWin()?.line?.[col] === row || (scatterWin() && (lastSpin?.grid || [])[row]?.[col] === 'SCATTER')); }
...
// Define paylineOverlayHtml to draw non-bonus win lines as an overlay anchored to the reel grid itself, so the line always crosses the winning cells. (issue #319)
