...
function renderFelt(roundItem) {
  ...
  return `<section class="panel game-stage" data-testid="blackjack-stage"><div class="row action-row"><div style="margin-right:auto"><span class="badge" data-testid="blackjack-round-id" data-round-id="${safe(roundItem?.round_id || '')}">${safe(roundItem ? T('stage.round', { id: roundItem.round_id }) : T('stage.noRound'))}</span><h2 style="margin:8px 0 0">${safe(roundTitle(roundItem))}</h2></div><span class="badge" data-testid="blackjack-stage-status">${safe(statusLabel(roundItem?.status))}</span></div><div class="result-box" data-testid="blackjack-felt" style="display:grid;grid-template-rows:auto 1fr auto;gap:16px;min-height:560px;padding:16px;background:linear-gradient(135deg, rgba(13,117,62,.82), rgba(4,64,36,.9));border-color:rgba(255,217,120,.55);">${renderDealer(roundItem)}<div style="display:grid;place-items:center;min-height:118px;"><div class="result-box" style="display:grid;place-items:center;width:min(70%,310px);min-height:96px;border-radius:50%;text-align:center;color:#fff2c2;"><b>${safe(logo)}</b><span class="muted">${safe(sublogo)}</span></div></div>${renderHandGrid(roundItem)}</div></section>`;
}
