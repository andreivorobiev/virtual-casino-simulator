...
let busy = false;
// Retain the last settled deal configuration so one click can repeat the same wager and mode.
let lastBet = null;
...
function controlsHtml(roundItem) {
  ...
  // Enable one-click repeat only outside a mid-hand round when a captured config exists and nothing is in flight.
  const repeatDisabled = Boolean(state?.active_round) || busy || Boolean(pendingRequestId) || !lastBet;
  // Return the complete control rail with stable primary-action placement.
  return `<aside class="mhvp-panel mhvp-controls" aria-label="${safe(text('controls.title'))}"><h2>${safe(text('controls.title'))}</h2><fieldset${configurationDisabled ? ' disabled' : ''}><legend>${safe(text('controls.mode'))}</legend><div class="mhvp-modes">${modes}</div></fieldset><label for="mhvp-wager">${safe(text('controls.wagerPerHand'))}</label><input id="mhvp-wager" type="number" min="0.01" max="100000" step="0.01" value="${safe(wagerPerHand)}"${configurationDisabled ? ' disabled' : ''}><p class="mhvp-total">${safe(text('controls.totalWager'))}: <strong>${safe(tokenAmount(handCount * wagerPerHand))}</strong></p><button type="button" class="mhvp-primary" data-action="deal"${dealDisabled ? ' disabled' : ''}>${safe(text('controls.deal'))}</button><button type="button" class="mhvp-repeat" data-action="repeat"${repeatDisabled ? ' disabled' : ''}>${safe(text('controls.repeat'))}</button><button type="button" data-action="draw"${drawDisabled ? ' disabled' : ''}>${safe(text('controls.draw'))}</button><p class="mhvp-help">${safe(text('controls.holdHelp'))}</p></aside>`;
}
...
function stylesHtml() {
  ...
  return `<style>
    ...
    .mhvp-shell{display:grid;gap:18px;min-width:0}.mhvp-header{display:flex;align-items:end;justify-content:space-between;gap:12px}.mhvp-header h1{margin:0}.mhvp-phase{padding:7px 12px;border:1px solid var(--gold);border-radius:999px}.mhvp-layout{display:grid;grid-template-columns:minmax(210px,.72fr) minmax(540px,2.4fr) minmax(230px,.8fr);gap:16px;align-items:start}.mhvp-panel,.mhvp-stage{border:1px solid var(--border);border-radius:16px;background:rgba(20,10,34,.82);padding:16px}.mhvp-controls{display:grid;gap:12px}.mhvp-controls fieldset{margin:0;padding:0;border:0}.mhvp-modes{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.mhvp-mode.is-selected{outline:2px solid var(--gold);outline-offset:2px}.mhvp-primary{background:#a51f2d;color:#fff}.mhvp-help,.mhvp-total,.mhvp-data p{color:var(--muted,#b8c8c1)}.mhvp-stage{display:grid;gap:16px;min-width:0}.mhvp-stage-kicker{margin:0 0 10px;text-align:center}.mhvp-card-row{display:flex;flex-wrap:wrap;justify-content:center;gap:8px}.mhvp-card-button{position:relative;min-width:58px;min-height:86px;padding:2px;border:2px solid transparent;background:transparent}.mhvp-card-button.is-held{border-color:var(--gold);border-radius:10px}.mhvp-held-label{position:absolute;inset:auto 3px 3px;padding:2px 5px;border-radius:999px;background:var(--felt);color:var(--gold);font-size:10px}.mhvp-summary{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.mhvp-summary span{padding:10px;border-radius:10px;background:rgba(255,255,255,.04)}.mhvp-results{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.mhvp-result{display:grid;gap:8px;padding:12px;border:1px solid rgba(255,255,255,.12);border-radius:12px}.mhvp-result header{display:flex;justify-content:space-between;gap:8px}.mhvp-result h3,.mhvp-result p{margin:0}.mhvp-result .playing-card{font-size:clamp(.65rem,1vw,.9rem)}.mhvp-data table{width:100%;border-collapse:collapse}.mhvp-data th,.mhvp-data td{padding:7px 4px;border-bottom:1px solid rgba(255,255,255,.1);text-align:left}.mhvp-empty{min-height:330px;display:grid;place-content:center;text-align:center}.mhvp-card-button:focus-visible,.mhvp-shell button:focus-visible,.mhvp-shell input:focus-visible{outline:3px solid var(--gold);outline-offset:2px}@media(max-width:1100px){.mhvp-layout{grid-template-columns:1fr}.mhvp-controls{order:1}.mhvp-stage{order:2}.mhvp-data{order:3}.mhvp-results{grid-template-columns:1fr}}@media(max-width:520px){.mhvp-header{align-items:start;flex-direction:column}.mhvp-panel,.mhvp-stage{padding:12px}.mhvp-summary{grid-template-columns:1fr}.mhvp-card-button{min-width:48px;min-height:72px}}@media(prefers-reduced-motion:reduce){.mhvp-shell *{scroll-behavior:auto!important;transition:none!important;animation:none!important}}.mhvp-repeat{min-height:46px;background:transparent;color:var(--gold);border:1px solid var(--gold)}.mhvp-repeat:disabled{opacity:.5}
...
async function draw() {
  ...
  state = payload.state;
  // Capture the settled deal configuration so one click can repeat the same mode and wager.
  lastBet = { hand_count: roundItem.hand_count, wager_per_hand: roundItem.wager_per_hand };
  // Refresh the shared authenticated wallet after any payout credit.
  await refreshBalance();
}


// Re-apply the last settled deal configuration and start one identical deal.
async function repeat() {
  // Ignore repeat while a request is in flight, a deal is unresolved, or no config was captured.
  if (busy || pendingRequestId || !lastBet) return;
  // Ignore repeat while a common hand is still mid-round awaiting a draw.
  if (state?.active_round) return;
  // Restore the captured hand mode so the repeated deal matches the prior round.
  handCount = lastBet.hand_count;
  // Restore the captured per-hand wager into cached configuration.
  wagerPerHand = lastBet.wager_per_hand;
  // Reflect the restored wager in the mounted input so deal reads the repeated stake.
  const input = root?.querySelector('#mhvp-wager');
  // Apply the stored wager only when the enabled wager control is present.
  if (input) input.value = String(wagerPerHand);
  // Start one idempotent deal with the restored mode and wager through shared busy handling.
...
function bindEvents() {
  ...
  // Wire the one-click repeat that re-opens a deal with the captured mode and wager.
  const repeatButton = root.querySelector('[data-action="repeat"]');
  // Re-apply the last settled configuration when the mounted button is available.
...
export const MultiHandVideoPokerGame = {
  ...
  async mount(node) {
    ...
    root = node;
    // Reset the repeatable config so a new session never inherits a prior deal.
    lastBet = null;
    ...
    // Recover a repeatable config from the newest settled round so repeat survives a reload.
    const recovered = state?.recent_rounds?.slice(-1)[0];
    // Restore the repeatable config only when a prior settled round exposes its mode and wager.
    if (recovered?.hand_count && recovered?.wager_per_hand) lastBet = { hand_count: recovered.hand_count, wager_per_hand: recovered.wager_per_hand };
  ...
  unmount() {
    ...
    state = null;
    // Clear the repeatable config so the next session starts fresh.
    lastBet = null;
