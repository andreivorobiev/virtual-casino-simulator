...
function ensureStyles() {
  ...
  style.textContent = '.dbvp{display:grid;grid-template-columns:minmax(0,1fr) 300px;gap:16px;width:100%;min-width:0;min-height:100%;color:var(--text,#f5ead6);align-items:start;} .db-stage{display:grid;gap:16px;padding:12px;min-width:0;justify-items:start;} .db-hand{display:flex;gap:8px;flex-wrap:wrap;min-width:0;} .db-slot{display:grid;justify-items:center;gap:4px;} .db-holdbtn{background:none;border:none;cursor:pointer;padding:0;border-radius:10px;} .db-holdbtn[aria-pressed="true"]{outline:2px solid #f2d77d;outline-offset:3px;} .db-tag{font-size:11px;font-weight:900;color:#f2d77d;text-transform:uppercase;min-height:14px;} .db-actions{display:flex;flex-wrap:wrap;gap:8px;} .db-btn{min-height:44px;padding:0 18px;border:none;border-radius:12px;font-weight:900;font-size:15px;cursor:pointer;} .db-btn.draw{background:linear-gradient(180deg,#0f9c4c,#0a5f2e);color:#fff;} .db-btn.deal{background:linear-gradient(180deg,#d6323d,#8e1822);color:#fff;width:100%;} .db-btn:disabled{opacity:.55;cursor:not-allowed;} .db-panel{display:grid;gap:12px;min-width:0;} .db-card{padding:14px;border:1px solid rgba(255,217,120,.42);border-radius:16px;background:rgba(0,0,0,.22);} .db-card h3{margin:0 0 10px;color:#e7bc52;text-transform:uppercase;font-size:12px;letter-spacing:.08em;} .db-field{display:grid;gap:4px;margin-bottom:10px;} .db-field label{font-size:12px;font-weight:700;} .db-field input{min-height:40px;border-radius:10px;border:1px solid rgba(255,255,255,.2);background:rgba(255,255,255,.05);color:#f5ead6;padding:0 10px;font-weight:800;} .db-pays{display:grid;gap:3px;font-size:12px;font-weight:700;} .db-pays div{display:flex;justify-content:space-between;} .db-pays span:last-child{color:#f2d77d;} .db-result{min-height:24px;font-size:15px;color:#fff2c2;font-weight:800;} .db-result .net{font-weight:900;} @media (max-width:900px){.dbvp{grid-template-columns:1fr;}}';
...
function render() {
  ...
  const stage = drawing ? drawStage(round) : round && round.phase === 'settled' ? settledStage(round) : idleStage();
  // Build the side panel with the bet input and the paytable.
  const panel = sidePanel(drawing);
  // Paint the whole route.
  root.innerHTML = `<section class="dbvp" data-testid="double-bonus-video-poker"><div class="db-stage">${stage}</div><div class="db-panel">${panel}</div></section>`;
...
function settledStage(round) {
  ...
  return `<div class="db-hand">${cards}</div><p class="db-result" data-testid="double-bonus-video-poker-result">${line}</p><div class="db-actions"><button class="db-btn deal" data-deal="1" type="button" ${busy ? 'disabled' : ''}>${safe(text('action.deal_again'))}</button></div>`;
}

// Build the side panel with the bet input and the paytable.
function sidePanel(drawing) {
  // Hide the bet input while a draw is pending.
  const wagerCard = drawing ? '' : `<div class="db-card"><h3>${safe(text('label.bet'))}</h3><div class="db-field"><label for="db-bet">${safe(text('label.bet'))}</label><input id="db-bet" data-bet type="number" min="1" step="1" value="${bet}"></div><button class="db-btn deal" data-deal="1" type="button" ${busy ? 'disabled' : ''}>${safe(text('action.deal'))}</button></div>`;
  // Build the paytable card.
  const paytable = `<div class="db-card"><h3>${safe(text('label.paytable'))}</h3><div class="db-pays">${paytableRows()}</div></div>`;
