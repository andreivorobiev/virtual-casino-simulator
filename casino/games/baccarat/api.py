...
def payload(state=None):
    ...
    return {"game": GAME_ID, "state": public, "player": players.get_player("human"), "players": players.list_players()}
...
def register(router):
    ...
    def settings(body, query):
        ...
        save_game_state(GAME_ID, state); return payload(state)
    ...
    def clear(body, query, bet_id):
        ...
        save_game_state(GAME_ID, state)
        # Return the computed value to the caller.
        return {"cleared": item, **payload(state)}
    ...
    def deal(body, query):
        ...
        # Return the computed value to the caller.
        return {"coup": coup, "settlements": settlements, "bot_bets": [], **payload(state)}
