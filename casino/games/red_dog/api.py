...
class LedgerAdapter:
    ...
    def find_action(self, player_id: str, intent: dict):
        ...
        for event in ledger.read_recent(player_id, 1_000_000):
            # Read structured details defensively for historical non-Red-Dog rows.
            details = event.get("details") or {}
            # Ignore rows without this exact derived ledger action id.
            if details.get("red_dog_action_id") != intent["action_id"]:
            ...
            if not matches:
                ...
                raise ConflictError("Red Dog ledger action conflicts with a committed event")
    ...
    def transact(self, intent: dict) -> dict:
        # Route wager and raise intents through shared insufficient-funds handling.
        ...
        # Route refund and payout intents through the shared credit path.
        ...
        raise ValidationError("Red Dog ledger direction is invalid")
