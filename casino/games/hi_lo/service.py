...
class CoreLedgerGateway:
    ...
    def find(self, player_id: str, action_id: str):
        ...
        return next((row for row in reversed(rows) if row.get("game") == GAME_ID and (row.get("details") or {}).get("hi_lo_action_id") == action_id), None)

    # Commit or recover one debit, payout, or refund exactly once locally.
        ...
        with _ACTION_LOCK:
            ...
            if existing is not None:
                ...
                if not matches:
                    ...
                    raise ConflictError("action_id was already used for a different Hi-Lo action")
                # Return immutable ledger proof and replay evidence.
                return existing, True
            # Add the stable action and fingerprint to complete audit details.
            event_details = {**details, "hi_lo_action_id": action_id, "request_fingerprint": fingerprint}
