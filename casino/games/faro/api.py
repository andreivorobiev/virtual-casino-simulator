...
"""Additive v1 router registration for the isolated Faro module built on the settlement core. (#146)"""
...
def build_game() -> SimpleWagerGame:
    # Compose the shared core with Faro's entropy, validator, resolver, and public bet catalog.
    return SimpleWagerGame(game_id=rules.GAME_ID, wager_transaction_type="FARO_WAGER_DEBIT", settlement_transaction_type="FARO_SETTLEMENT_CREDIT", entropy=engine.entropy, resolve=engine.resolve, validate_bet=engine.validate_bet, public_bet_catalog=rules.bet_catalog)
...
def register(router, service=None):
    ...
    @router.get(r"/api/v1/games/faro/state")
    ...
    @router.post(r"/api/v1/games/faro/deals")
    # Execute or replay the complete deal for the bound player.
    def deals(body, query):
