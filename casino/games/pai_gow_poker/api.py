"""Additive v1 route registration for the isolated Pai Gow Poker module."""
...
def register(router, service=None, *, test_seed=None):
    ...
    game_service = service or PaiGowPokerService(seed_factory=(lambda action_id: f"{test_seed}:{action_id}") if test_seed is not None else None)

    # Register the player-scoped reload-safe state endpoint.
    @router.get(r"/api/v1/games/pai-gow-poker/state")
    ...
    # Register one idempotent ante deal awaiting a set decision.
    @router.post(r"/api/v1/games/pai-gow-poker/rounds")
    # Create or replay a prepared setting round.
    ...
    # Register one idempotent set decision.
    @router.post(r"/api/v1/games/pai-gow-poker/rounds/(?P<round_id>[A-Za-z0-9_-]+)/decisions")
    # Reveal the house-way dealer and settle the active bound-player round.
