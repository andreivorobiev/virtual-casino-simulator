...
class CoreLedgerGateway:
    ...
    def find(self, player_id: str, action_id: str):
        ...
        return next((row for row in reversed(rows) if row.get("game") == GAME_ID and (row.get("details") or {}).get("four_card_poker_action_id") == action_id), None)
        ...
        with _ACTION_LOCK:
            ...
            if existing is not None:
                ...
                if not matches:
                    ...
                    raise ConflictError("action_id was already used for a different Four Card Poker action")
                # Return immutable ledger proof and replay evidence.
                return existing, True
            # Add the stable action and fingerprint to complete audit details.
            event_details = {**details, "four_card_poker_action_id": action_id, "request_fingerprint": fingerprint}
