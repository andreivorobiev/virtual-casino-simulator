...
"""Additive v1 router registration for the isolated Marble Race module on the settlement core. (#157)"""
...
def build_game() -> SimpleWagerGame:
    # Compose the shared core with Marble Race's entropy, validator, resolver, and public bet catalog.
    return SimpleWagerGame(game_id=rules.GAME_ID, wager_transaction_type="MARBLE_RACE_WAGER_DEBIT", settlement_transaction_type="MARBLE_RACE_SETTLEMENT_CREDIT", entropy=engine.entropy, resolve=engine.resolve, validate_bet=engine.validate_bet, public_bet_catalog=rules.bet_catalog)
...
def register(router, service=None):
    ...
    @router.get(r"/api/v1/games/marble-race/state")
    ...
    # Register one atomic idempotent wager, race, and settlement action.
    @router.post(r"/api/v1/games/marble-race/races")
    # Execute or replay the complete race for the bound player.
    def races(body, query):
