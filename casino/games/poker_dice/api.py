...
"""Additive v1 router registration for the isolated Poker Dice module built on the settlement core. (#151)"""
...
def build_game() -> SimpleWagerGame:
    # Compose the shared core with Poker Dice's entropy, validator, resolver, and public paytable.
    return SimpleWagerGame(game_id=rules.GAME_ID, wager_transaction_type="POKER_DICE_WAGER_DEBIT", settlement_transaction_type="POKER_DICE_SETTLEMENT_CREDIT", entropy=engine.entropy, resolve=engine.resolve, validate_bet=engine.validate_bet, public_bet_catalog=rules.bet_catalog)
...
def register(router, service=None):
    ...
    @router.get(r"/api/v1/games/poker-dice/state")
    ...
    @router.post(r"/api/v1/games/poker-dice/rolls")
