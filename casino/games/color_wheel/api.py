...
"""Additive v1 router registration for the isolated Color Wheel module built on the settlement core. (#152)"""
...
def build_game() -> SimpleWagerGame:
    # Compose the shared core with Color Wheel's entropy, validator, resolver, and public catalog.
    return SimpleWagerGame(game_id=rules.GAME_ID, wager_transaction_type="COLOR_WHEEL_WAGER_DEBIT", settlement_transaction_type="COLOR_WHEEL_SETTLEMENT_CREDIT", entropy=engine.entropy, resolve=engine.resolve, validate_bet=engine.validate_bet, public_bet_catalog=rules.bet_catalog)
...
def register(router, service=None):
    ...
    @router.get(r"/api/v1/games/color-wheel/state")
    ...
    # Register one atomic idempotent wager, spin, and settlement action.
    @router.post(r"/api/v1/games/color-wheel/spins")
    # Execute or replay the complete spin for the bound player.
    def spins(body, query):
