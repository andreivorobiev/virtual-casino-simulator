"""Additive v1 router registration for the isolated Pattern Draw module on the settlement core. (#155)"""
...
def build_game() -> SimpleWagerGame:
    # Compose the shared core with Pattern Draw's entropy, validator, resolver, and public bet catalog.
    return SimpleWagerGame(game_id=rules.GAME_ID, wager_transaction_type="PATTERN_DRAW_WAGER_DEBIT", settlement_transaction_type="PATTERN_DRAW_SETTLEMENT_CREDIT", entropy=engine.entropy, resolve=engine.resolve, validate_bet=engine.validate_bet, public_bet_catalog=rules.bet_catalog)
...
def register(router, service=None):
    ...
    @router.get(r"/api/v1/games/pattern-draw/state")
    ...
    @router.post(r"/api/v1/games/pattern-draw/draws")
