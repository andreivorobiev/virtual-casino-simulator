"""Additive v1 router registration for the isolated Coin Pusher module on the settlement core. (#156)"""
...
def build_game() -> SimpleWagerGame:
    # Compose the shared core with Coin Pusher's entropy, validator, resolver, and public payout catalog.
    return SimpleWagerGame(game_id=rules.GAME_ID, wager_transaction_type="COIN_PUSHER_WAGER_DEBIT", settlement_transaction_type="COIN_PUSHER_SETTLEMENT_CREDIT", entropy=engine.entropy, resolve=engine.resolve, validate_bet=engine.validate_bet, public_bet_catalog=rules.bet_catalog)
...
def register(router, service=None):
    ...
    @router.get(r"/api/v1/games/coin-pusher/state")
    ...
    # Register one atomic idempotent wager, drop, and settlement action.
    @router.post(r"/api/v1/games/coin-pusher/drops")
    # Execute or replay the complete drop for the bound player.
    def drops(body, query):
