"""Additive v1 router registration for the isolated Lucky Grid module on the settlement core. (#153)"""
...
def build_game() -> SimpleWagerGame:
    # Compose the shared core with Lucky Grid's entropy, validator, resolver, and public bet catalog.
    return SimpleWagerGame(game_id=rules.GAME_ID, wager_transaction_type="LUCKY_GRID_WAGER_DEBIT", settlement_transaction_type="LUCKY_GRID_SETTLEMENT_CREDIT", entropy=engine.entropy, resolve=engine.resolve, validate_bet=engine.validate_bet, public_bet_catalog=rules.bet_catalog)
...
def register(router, service=None):
    ...
    @router.get(r"/api/v1/games/lucky-grid/state")
    ...
    # Register one atomic idempotent wager, reveal, and settlement action.
    @router.post(r"/api/v1/games/lucky-grid/reveals")
    # Execute or replay the complete reveal for the bound player.
    def reveals(body, query):
