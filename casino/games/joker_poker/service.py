...
class CoreLedgerGateway:
    ...
    def find(self, player_id: str, action_id: str):
        ...
        return next((row for row in reversed(rows) if row.get("game") == GAME_ID and (row.get("details") or {}).get("joker_poker_action_id") == action_id), None)
    ...
    def apply_once(self, *, player_id: str, signed_amount: float, transaction_type: str, round_id: str, action_id: str, fingerprint: str, details: dict) -> tuple[dict, bool]:
        ...
        with _ACTION_LOCK:
            ...
            if existing is not None:
                ...
                if not matches:
                    # Fail before any second balance mutation.
                    raise ConflictError("action_id was already used for a different Joker Poker action")
                # Return immutable ledger proof and replay evidence.
                return existing, True
            # Add the stable action and fingerprint to complete audit details.
            event_details = {**details, "joker_poker_action_id": action_id, "request_fingerprint": fingerprint}
