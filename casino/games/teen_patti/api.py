"""Additive v1 route registration for the isolated Teen Patti Practice module (#150)."""
...
from casino.games.teen_patti.service import TeenPattiService
...
def register(router, service=None, *, test_seed=None):
    ...
    game_service = service or TeenPattiService(seed_factory=(lambda action_id: f"{test_seed}:{action_id}") if test_seed is not None else None)

    # Register the player-scoped reload-safe state endpoint.
    @router.get(r"/api/v1/games/teen-patti/state")
    ...
    @router.post(r"/api/v1/games/teen-patti/rounds")
    ...
    @router.post(r"/api/v1/games/teen-patti/rounds/(?P<round_id>[A-Za-z0-9_-]+)/decisions")
