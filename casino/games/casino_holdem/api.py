...
"""Additive v1 route registration for the isolated Casino Hold'em module."""
...
def register(router, service=None, *, test_seed=None):
    ...
    game_service = service or CasinoHoldemService(seed_factory=(lambda action_id: f"{test_seed}:{action_id}") if test_seed is not None else None)

    # Register the player-scoped reload-safe state endpoint.
    @router.get(r"/api/v1/games/casino-holdem/state")
    ...
    # Register one idempotent ante deal through the flop.
    @router.post(r"/api/v1/games/casino-holdem/rounds")
    # Create or replay a prepared call-or-fold round.
    ...
    # Register one idempotent call-or-fold decision.
    @router.post(r"/api/v1/games/casino-holdem/rounds/(?P<round_id>[A-Za-z0-9_-]+)/decision")
    # Reveal and settle the active bound-player round when called, or fold it.
    def decision(body, query, round_id, context=None):
