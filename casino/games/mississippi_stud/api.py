"""Additive v1 route registration for the isolated Mississippi Stud module (#143)."""
...
from casino.games.mississippi_stud.service import MississippiStudService
...
def register(router, service=None, *, test_seed=None):
    ...
    game_service = service or MississippiStudService(seed_factory=(lambda action_id: f"{test_seed}:{action_id}") if test_seed is not None else None)

    # Register the player-scoped reload-safe state endpoint.
    @router.get(r"/api/v1/games/mississippi-stud/state")
    ...
    # Register one idempotent ante deal.
    @router.post(r"/api/v1/games/mississippi-stud/rounds")
    # Create or replay a prepared street-decision round.
    ...
    # Register one idempotent bet-or-fold street decision.
    @router.post(r"/api/v1/games/mississippi-stud/rounds/(?P<round_id>[A-Za-z0-9_-]+)/decisions")
    # Bet one to three times the ante at the current street, or fold the round.
