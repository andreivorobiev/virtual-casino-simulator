"""Additive v1 router registration for the isolated Trente et Quarante module on the core. (#147)"""
...
def build_game() -> SimpleWagerGame:
    # Compose the shared core with this game's entropy, validator, resolver, and public bet catalog.
    return SimpleWagerGame(game_id=rules.GAME_ID, wager_transaction_type="TRENTE_ET_QUARANTE_WAGER_DEBIT", settlement_transaction_type="TRENTE_ET_QUARANTE_SETTLEMENT_CREDIT", entropy=engine.entropy, resolve=engine.resolve, validate_bet=engine.validate_bet, public_bet_catalog=rules.bet_catalog)
...
def register(router, service=None):
    ...
    @router.get(r"/api/v1/games/trente-et-quarante/state")
    ...
    # Register one atomic idempotent wager, deal, and settlement action.
    @router.post(r"/api/v1/games/trente-et-quarante/coups")
    # Execute or replay the complete coup for the bound player.
    def coups(body, query):
