...
**Historical.** Records the #92 pre-integration slice and its handoff to #77. Deuces Wild Video Poker has since been integrated; for current state see `modules/deuces_wild_video_poker.json`, `modules/module-manifest.json`, and `docs/requirements/requirements.json`.
...
- The canonical `1.0.1` descriptor is installed at `modules/deuces_wild_video_poker.json` and its revision is recorded in `modules/module-manifest.json`.
...
## Permanent requirement block

These identifiers were allocated by #77 and are now permanent entries in `docs/requirements/requirements.json` at status `PASS`, mapped to real-backend acceptance evidence.

| ID | Requirement | Existing requirement mapping |
| --- | --- | --- |
...
`USER-003`, `TOKEN-001`, and `TEST-039` remain `PLANNED` until their own acceptance boundaries are satisfied; `DWVP-001` through `DWVP-005` are registered at `PASS`. The existing `MHVP-001` through `MHVP-005` identifiers belong only to Multi-Hand Video Poker and are not reused here.
...
1. `modules/deuces_wild_video_poker.json` and `modules/module-manifest.json`: promote the accepted proposal to the installable descriptor path and add its canonical revision in the same #77 integration change, then recalculate any shared module revisions from the then-current main branch.
...
The runtime scans `modules/*.json`; the descriptor and its aggregate manifest revision were promoted atomically by #77, so catalog lookup, reset, and long-suite discovery all resolve Deuces Wild. Promoting a descriptor without its aggregate revision would make runtime catalog lookup fail, which is why the two had to land together. Focused engine, isolated-router API, frontend, locale-parity, boundary, comment-density, and syntax checks remain valid worker evidence but do not remove that integration gate.
