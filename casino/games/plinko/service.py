...
class CoreLedgerGateway:
    ...
    def find(self, player_id: str, action_id: str):
        ...
        return next((row for row in reversed(rows) if row.get("game") == GAME_ID and (row.get("details") or {}).get("plinko_action_id") == action_id), None)

    # Commit or recover one debit or payout exactly once locally.
    def apply_once(self, *, player_id: str, signed_amount: float, transaction_type: str, drop_id: str, action_id: str, fingerprint: str, details: dict) -> tuple[dict, bool]:
        ...
        with _ACTION_LOCK:
            ...
            if existing is not None:
                # Compare movement, route stage, drop, and semantic request content.
                matches = round(float(existing.get("amount", 0)), 2) == round(float(signed_amount), 2) and existing.get("transaction_type") == transaction_type and existing.get("round_id") == drop_id and (existing.get("details") or {}).get("request_fingerprint") == fingerprint
                # Reject one client identity reused for a different action.
                if not matches:
                    # Fail closed before any second balance mutation.
                    raise ConflictError("action_id was already used for a different Plinko action")
                # Return immutable ledger proof and replay evidence.
                return existing, True
            # Add the stable action and fingerprint to complete audit details.
            event_details = {**details, "plinko_action_id": action_id, "request_fingerprint": fingerprint}
            # Route negative amounts through the approved debit service.
            if signed_amount < 0:
                # Commit the positive magnitude as one shared-ledger debit.
                event = self._debit(player_id, abs(signed_amount), transaction_type, GAME_ID, drop_id, event_details)
            # Route positive returned tokens through the approved credit service.
                ...
                event = self._credit(player_id, signed_amount, transaction_type, GAME_ID, drop_id, event_details)
