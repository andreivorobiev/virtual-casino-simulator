...
"""Additive v1 router registration for the isolated Pachinko module built on the settlement core. (#142)"""
...
def build_game() -> SimpleWagerGame:
    # Compose the shared core with Pachinko's entropy, validator, resolver, and public payout catalog.
    return SimpleWagerGame(game_id=rules.GAME_ID, wager_transaction_type="PACHINKO_WAGER_DEBIT", settlement_transaction_type="PACHINKO_SETTLEMENT_CREDIT", entropy=engine.entropy, resolve=engine.resolve, validate_bet=engine.validate_bet, public_bet_catalog=rules.bet_catalog)
...
def register(router, service=None):
    ...
    @router.get(r"/api/v1/games/pachinko/state")
    ...
    @router.post(r"/api/v1/games/pachinko/drops")
