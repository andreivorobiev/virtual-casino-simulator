...
class PasswordResetService:
    ...
    def complete(self, token: str, email: str, new_password: str, *, idempotency_key: str = "") -> dict:
        ...
        auth.set_user_password(user["user_id"], str(new_password))
        # Revoke every existing session so a stolen session cannot survive a recovery.
        revoked = auth.revoke_sessions_for_user(user["user_id"])
        ...
        return {"status": "reset", "sessions_revoked": revoked}
