...
import time
# Import generic callable, connection, and structural-adapter types without binding to psycopg.
...
class PostgresPoolConnectionError(RuntimeError):
...
# Define the connector-neutral operations required to sanitize and validate physical sessions.
class PostgresConnectionAdapter(Protocol):
    # Return whether the same physical session answers a wire check without reconnecting.
    def is_healthy(self, connection: Any) -> bool:
        # Leave the implementation to the later provider-owned psycopg adapter.
        ...

    # Return whether the physical session has non-idle transaction state.
    def in_transaction(self, connection: Any) -> bool:
        # Leave connector transaction-state interpretation outside this pool.
        ...

    # Roll back one non-idle physical session before reuse.
    def rollback(self, connection: Any) -> None:
        # Leave connector rollback mechanics outside this pool.
        ...

    # Restore one idle physical session to the provider's reviewed baseline.
    def reset(self, connection: Any) -> None:
        # Leave connector session-reset mechanics outside this pool.
        ...


# Define the validated per-process PostgreSQL pool policy.
...
class PostgresPoolConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.capacity, bool) or not isinstance(self.capacity, int) or not 1 <= self.capacity <= 64:
            # Raise a fixed validation message that contains no environment value.
            raise ValueError("PostgreSQL pool capacity must be between 1 and 64.")
        # Reject unbounded or non-positive checkout waits.
        if isinstance(self.checkout_wait_ms, bool) or not isinstance(self.checkout_wait_ms, int) or not 1 <= self.checkout_wait_ms <= 10_000:
            # Raise a fixed validation message that contains no environment value.
            raise ValueError("PostgreSQL pool checkout wait must be between 1 and 10000 milliseconds.")
...
class PostgresConnectionLease:
    # Store the pool, physical connection, process identity, and pool generation for one checkout.
    def __init__(self, pool: PostgresConnectionPool, connection: Any, owner_pid: int, generation: int) -> None:
    ...
    def cursor(self, *args: Any, **kwargs: Any) -> Any:
        ...
        with self._close_lock:
            ...
            if self._closed:
                ...
                raise PostgresPoolClosedError("PostgreSQL connection lease is closed.")
    ...
    def __enter__(self) -> PostgresConnectionLease:
...
# Define the bounded process-local PostgreSQL physical connection pool.
class PostgresConnectionPool:
    ...
    def __init__(
        ...
        # Store the injected connector-neutral lifecycle adapter.
        self._adapter = adapter
        # Store validated non-secret policy derived from explicit config or environment.
        self.config = config or PostgresPoolConfig.from_env()
    ...
    def _ensure_process(self) -> None:
        ...
        for connection in inherited_idle:
            # Ensure the child never reuses a parent-created PostgreSQL session.
    ...
    def _is_healthy(self, connection: Any) -> bool:
        # Start the protected wire check because a dead socket is an expected discard path.
        try:
            # Require the adapter to check this exact physical session without replacing it.
            healthy = self._adapter.is_healthy(connection)
        # Treat every connector failure as an unhealthy connection without retaining its details.
            ...
            return False
        # Return healthy only when the adapter explicitly confirms the wire check.
        return healthy is True
    ...
    def acquire(self, connect_timeout_seconds: int | None = None) -> PostgresConnectionLease:
        ...
        if isinstance(physical_timeout, bool) or not isinstance(physical_timeout, int) or not 1 <= physical_timeout <= 60:
        ...
        while True:
            ...
            with self._condition:
                ...
                if self._closed:
                    ...
                    raise PostgresPoolClosedError("PostgreSQL connection pool is closed.")
                ...
                while self._idle:
                    ...
                    if self._is_healthy(connection):
                        ...
                        return PostgresConnectionLease(self, connection, self._pid, self._generation)
                ...
                if remaining <= 0:
                    ...
                    raise PostgresPoolExhaustedError("PostgreSQL connection pool checkout timed out.")
        ...
        # Convert every connector-specific failure into one fixed internal error.
            ...
            raise PostgresPoolConnectionError("PostgreSQL physical connection could not be created.") from None
    ...
    def release(self, connection: Any, generation: int, force_discard: bool = False) -> None:
        ...
        try:
            # Ask the adapter whether connector-specific transaction state is non-idle.
            if self._adapter.in_transaction(connection):
                # End the caller-abandoned transaction before reuse.
                self._adapter.rollback(connection)
            ...
            if reusable:
                ...
                self._adapter.reset(connection)
