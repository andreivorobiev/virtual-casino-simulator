...
PUBLIC_API_PATHS = {"/api/v2/auth/login", "/api/v2/auth/oauth/providers", "/healthz"}
...
def create_session(user: dict, client: str = "", auth_method: str = "local") -> dict:
    ...
    # Store the authentication method for provider rollback and safe unlink revocation.
...
def revoke_sessions_for_user_method(user_id: str, auth_method: str) -> int:
    ...
    def mutate(state: dict) -> dict:
        # Normalize malformed state before applying the bounded revocation.
        if not isinstance(state, dict) or "sessions" not in state:
...
def is_public_api_path(path: str) -> bool:
    ...
    if path in PUBLIC_API_PATHS:
        # Allow login, provider availability, and liveness without a session.
        return True
    # Allow only exact reviewed OAuth start and callback route shapes.
